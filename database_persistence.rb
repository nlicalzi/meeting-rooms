...
# This class abstracts the logic of interacting with the database from the business logic
# in room_booker.rb making it more simple to switch databases eventually if necessary.
class DatabasePersistence
  ...
  def get_conflicting_meetings(room_id, date, mtg_start, mtg_end)
    sql = 'SELECT * FROM meetings WHERE room_id = $1 AND mtg_date = $2 '\
          'AND (start_time, end_time) OVERLAPS ($3::time, $4::time);'
    query(sql, room_id, date, mtg_start, mtg_end).entries
  end

  def get_available_rooms(date, mtg_start, mtg_end)
    subquery = 'SELECT room_id FROM meetings WHERE mtg_date = $1 '\
